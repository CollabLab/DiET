...
/**
 * 
 * <p>This is a customized version of Infonode's implementation of TitledTab which permits
 * the Font of the TitledTab to be changed.
 * 
 ...
 */
public class TitledTabWithFontSettings extends Tab implements IconProvider {
  ...
  private class StatePanel extends SimplePanel {
    ...
    public StatePanel(Border focusBorder) {
      ...
      if(fontBold)label.setFont(label.getFont().deriveFont(Font.BOLD));
      //label.setFont(null);
  ...
  private class HoverablePanel extends SimplePanel implements Hoverable {
    ...
    public void hoverEnter() {
      if (hoverListener != null && getTabbedPanel() != null)
        hoverListener.mouseEntered(new HoverEvent(TitledTabWithFontSettings.this));
    }

    public void hoverExit() {
      if (hoverListener != null)
        hoverListener.mouseExited(new HoverEvent(TitledTabWithFontSettings.this));
    }
  ...
  public TitledTabWithFontSettings(String text, Icon icon, JComponent contentComponent, JComponent titleComponent) {
      this(text, icon,  contentComponent,  titleComponent,  12);
  }
  
  
  public TitledTabWithFontSettings(String text, Icon icon, JComponent contentComponent, JComponent titleComponent, int fontSize) {
    ...
    eventPanel.addMouseListener(new MouseAdapter() {
      ...
      private void updateFocus(TabSelectTrigger trigger) {
        if (isEnabled() && properties.getFocusable() && getTabbedPanel() != null && getTabbedPanel().getProperties()
          ...
          if (focusedComponent instanceof TitledTabWithFontSettings
              && ((TitledTabWithFontSettings) focusedComponent).getTabbedPanel() == getTabbedPanel())
  ...
  private void updateHoverListener(HoverListener newHoverListener) {
    ...
    if (HoverManager.getInstance().isHovered(eventPanel)) {
      if (oldHoverListener != null)
        oldHoverListener.mouseExited(new HoverEvent(TitledTabWithFontSettings.this));
      if (hoverListener != null)
        hoverListener.mouseEntered(new HoverEvent(TitledTabWithFontSettings.this));
    }
  }
  ...
  private MouseEvent convertMouseEvent(MouseEvent e) {
    Point p = SwingUtilities.convertPoint((JComponent) e.getSource(), e.getPoint(), TitledTabWithFontSettings.this);
    return new MouseEvent(TitledTabWithFontSettings.this, e.getID(), e.getWhen(), e.getModifiers(),
