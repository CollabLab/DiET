 ...
 * @author sre
 */
public class CCPICTURESTIMULI6 extends DefaultConversationController{
    ...
    public void processLoop() {
        super.processLoop();
        if(mode==CCPICTURESTIMULI6.mode5TRAININGPHASE)c.resetFromIsTypingtoNormalChatAllAllowedParticipants(super.getIsTypingTimeOut());
    ...
    public void processChatText(Participant sender, MessageChatTextFromClient mct) {
        ...
        if(mode==CCPICTURESTIMULI6.mode5TRAININGPHASE&&mct.getText().startsWith("/")){
           this.processSelection(sender, mct.getText());
        }
        else if (mode==CCPICTURESTIMULI6.mode5TRAININGPHASE){
...
public synchronized void initiateFirstTestPhaseDirector(final int startindex, final int numberoftrials){
    ...
    Thread t = new Thread(){
        public void run(){
        mode=CCPICTURESTIMULI6.mode1TESTPHASE_DIRECTOR;
        ...
        mode=CCPICTURESTIMULI6.mode2TESTPHASE_POSTDIRECTOR_PREMATCHER;
...
public synchronized void initiateFirstTestPhaseMatcher(final int startindex, final int numberoftrials, final int returnindex){
        ...
        mode=CCPICTURESTIMULI6.mode3TESTPHASE_MATCHER;
        ...
        mode = CCPICTURESTIMULI6.mode4POSTTESTPHASE;
...
public synchronized void initiateSecondTestPhaseDirector(final int startindex, final int numberoftrials){
    ...
    Thread t = new Thread(){
        public void run(){
        mode=CCPICTURESTIMULI6.mode7TESTPHASE_DIRECTOR;
        ...
        mode=CCPICTURESTIMULI6.mode8TESTPHASE_POSTDIRECTOR_PREMATCHER;
...
public synchronized void initiateSecondTestPhaseMatcher(final int startindex, final int numberoftrials, final int returnindex){
        Thread t = new Thread(){
        public void run(){
        mode = CCPICTURESTIMULI6.mode9TESTPHASE_MATCHER;
        for(int i=startindex;i<startindex+numberoftrials;i++){
                  singleMatcherTrial(i);
                  try { Thread.sleep(pauseBetweenSetsMatcherTrial);} catch (Exception e){ e.printStackTrace();}   
        }
