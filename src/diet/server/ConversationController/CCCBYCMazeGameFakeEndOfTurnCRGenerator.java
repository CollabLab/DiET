...
public class CCCBYCMazeGameFakeEndOfTurnCRGenerator extends CCCBYCMazeGameFakeEdit{
   ...
   public void fNI(){
    ...
    public boolean isCR_OK(Participant p){
        ...
        int currentTraversalCount = mgc.getSwitchTraversalCount(p.getUsername())+mgc.getMazeNo(); //To be able to generate intervention at beginning of each game
        ...
        if(idx==0){
           ...
           // Conversation.printWSln("Main","Can perform intervention participant 1: "+ participant1mazeInterventions[mgc.getMazeNo()]+"--"+maxInterventionsPerMaze.getValue());
            return true;
        }
        else if(idx==1){
            ...
            //Conversation.printWSln("Main","Can perform intervention participant 2: "+ participant2mazeInterventions[mgc.getMazeNo()]+"--"+maxInterventionsPerMaze.getValue());
            return true;
        }
   ...
   boolean blockallinterventions = true;
   

   public void allowI(){
       this.blockallinterventions= false;
   }

   public void blockI(){
       this.blockallinterventions=true;
   }
    ...
    public void processCBYCDocChange(Participant sender, MessageCBYCDocChangeFromClient mCDC) {
        if(Debug.inCRENDOFTURNJUMPTOMAZE6){
            //mgc  = (MazeGameController2WAY)this.getC().getTaskController();
           //if(mgc.getMazeNo()<6)mgc.moveToMazeNo(6);
        }
    ...
    public synchronized Sequence getNextSequence_Speaker_Change(Sequence prior,int sequenceNo, Sequences ss, Participant sender, MessageCBYCTypingUnhinderedRequest mCTUR) {
        ...
        if(state==2){
            ...
            this.mostRecentTextCONTIG="";Conversation.printWSln("Main3", "RESETTINGCONTIGSPEAKERCHANGE");
            return mgcSWFT;
            }
        }
         ...
         if(speakerChanges>3&&this.getC().getParticipants().getAllParticipants().size()>1){
               ...
               int rnd = 2;//r.nextInt(super.interventionEvery.getValue());
               ...
               this.mostRecentTextCONTIG="";Conversation.printWSln("Main3", "RESETTINGCONTIGSPEAKERCHANGE_AFTERCHECKINGFORCR");
...
///////////////////////////////////////////////////////////////////////////////
                   ...
                   String cr = "";//"+this.possCR+"?";possCR=""; //this.cycCRIN_participant1.getNext(pRecipient);
                  
                   if(this.interventionFRAG){
                       cr = ""+this.possCR+"?";possCR="";
                   }
                   else if(!this.interventionFRAG){
    ...
    public Sequence getNextSequence_NewLine_By_Same_Speaker(Sequence prior, int sequenceNo, Sequences sS, String sender, MessageCBYCDocChangeFromClient mCDC) {
         this.mostRecentTextCONTIG= mostRecentTextCONTIG + " "+this.mostRecentText;Conversation.printWSln("Main3", "ADDING NEWLINECONTIG....["+this.mostRecentTextCONTIG+"] ");
         ...
         try{
             //Conversation.printWSln("Main", "IT'S sleeping");
             //Thread.sleep(8000);
         }catch(Exception e){
   ...
   public Sequence getNextSequence_Edit_By_Different_Speaker(Sequence prior,int sequenceNo,Sequences ss,Participant p,MessageCBYCTypingUnhinderedRequest mCTUR){
       this.mostRecentTextCONTIG="";Conversation.printWSln("Main3", "RESETTINGCONTIG");
       String sendername = prior.getSender();
        if(state==2||state==3){
