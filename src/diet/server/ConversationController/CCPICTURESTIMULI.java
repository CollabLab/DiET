 ...
 * @author sre
 */
public class CCPICTURESTIMULI extends DefaultConversationController{

    String sbfolder = "stimuliset9";
    ...
    public synchronized String gotoSet(int srch, boolean alternateDM){
        ...
        c.saveDataToFile("DATA", "server", "server", new Date().getTime(), new Date().getTime(), director.getUsername()+":"+directorFile.getName()+"----"+matcher.getUsername()+"----"+matcherFile.getName(), null);
    ...
    /*     Vector v = new Vector();
         ...
         ////
      */  
         //(firsttestlength + this.secondtestlength)
        
        
         currSetDirector = srch;
         currSetMatcher= (firsttestlength + this.secondtestlength)+srch;
        ...
        String numM =""+((firsttestlength + this.secondtestlength)+srch);
        ...
        c.saveDataToFile("DATA(2D)", "server", "server", new Date().getTime(), new Date().getTime(), director.getUsername()+":"+directorFile.getName()+"----"+matcher.getUsername()+"----"+matcherFile.getName(), null);
    ...
    public synchronized String gotoSet2M(int srch){
         currSetDirector = ((firsttestlength + this.secondtestlength)+srch);
         currSetMatcher=   srch;
        ...
        String numD =""+ ((firsttestlength + this.secondtestlength)+srch);
    ...
    public void processLoop() {
        super.processLoop();
        if(mode==CCPICTURESTIMULI.mode5TRAININGPHASE)c.resetFromIsTypingtoNormalChatAllAllowedParticipants(super.getIsTypingTimeOut());
    ...
    public void processChatText(Participant sender, MessageChatTextFromClient mct) {
        ...
        if(mode==CCPICTURESTIMULI.mode5TRAININGPHASE&&mct.getText().startsWith("/")){
           this.processSelection(sender, mct.getText());
           c.saveDataToFile("SELECTION", sender.getParticipantID(), sender.getUsername(), new Date().getTime(),  new Date().getTime(), mct.getText(), null);
        }
        else if (mode==CCPICTURESTIMULI.mode5TRAININGPHASE){
...
public synchronized void initiateFirstTestPhaseDirector(final int startindex, final int numberoftrials){
    ...
    Thread t = new Thread(){
        public void run(){
        mode=CCPICTURESTIMULI.mode1TESTPHASE_DIRECTOR;
        ...
        mode=CCPICTURESTIMULI.mode2TESTPHASE_POSTDIRECTOR_PREMATCHER;
...
public synchronized void initiateFirstTestPhaseMatcher(final int startindex, final int numberoftrials, final int returnindex){
        ...
        mode=CCPICTURESTIMULI.mode3TESTPHASE_MATCHER;
        ...
        mode = CCPICTURESTIMULI.mode4POSTTESTPHASE;
...
public synchronized void initiateSecondTestPhaseDirector(final int startindex, final int numberoftrials){
    ...
    Thread t = new Thread(){
        public void run(){
        mode=CCPICTURESTIMULI.mode7TESTPHASE_DIRECTOR;
        ...
        mode=CCPICTURESTIMULI.mode8TESTPHASE_POSTDIRECTOR_PREMATCHER;
...
public synchronized void initiateSecondTestPhaseMatcher(final int startindex, final int numberoftrials, final int returnindex){
        Thread t = new Thread(){
        public void run(){
        mode = CCPICTURESTIMULI.mode9TESTPHASE_MATCHER;
        for(int i=startindex;i<startindex+numberoftrials;i++){
                  singleMatcherTrial(i);
                  try { Thread.sleep(pauseBetweenSetsMatcherTrial);} catch (Exception e){ e.printStackTrace();}   
        }
...
public synchronized void singleMatcherTrial(int startIndex){
    ...
    String chOutPutForD =director.getUsername()+" is now looking at "+ this.directorFile.getName()+"..having to select "+this.directorcorrectFaceNumber+ " using this description:"+ stringForDNoNewlines.replaceAll("\\r\\n|\\r|\\n", "(NEWLINE)");
    c.saveDataToFile("WHOISLOOKINGAT(M)", "server", "server", new Date().getTime(), new Date().getTime(), chOutPutForD, null);
    for(int i = 0; i < sDArray.length;i++){
     ...
     String chOutPutForM = matcher.getUsername()+" is now looking at "+ this.matcherFile.getName()+"..having to select "+matchercorrectFaceNumber+ " using this description:"+ stringForMNoNewlines.replaceAll("\\r\\n|\\r|\\n", "(NEWLINE)");
     c.saveDataToFile("WHOISLOOKINGAT(M)", "server", "server", new Date().getTime(), new Date().getTime(), chOutPutForM, null);
    ...
    System.err.println("(A)SINGLEMATCHERTRIALLOOKINGFOR: "+stringForM+"------"+matcherFile.getName()+currSetMatcher);
    System.err.println("(B)SINGLEMATCHERTRIALLOOKINGFOR: "+stringForD+"------"+directorFile.getName()+currSetDirector);
...
public synchronized void singleDirectorTrial(int startIndex){
    ...
    public synchronized void processWYSIWYGTextInserted(Participant sender, MessageWYSIWYGDocumentSyncFromClientInsert mWYSIWYGkp) {
        ...
        if(sender==director && (this.mode==this.mode1TESTPHASE_DIRECTOR||this.mode==this.mode7TESTPHASE_DIRECTOR)){
            ...
            c.saveDataToFile("DIRECTORCRIPTIONMAKING", sender.getParticipantID(), sender.getUsername() , new Date().getTime(), new Date().getTime(),newLineRemoved, null);       
            ...
            c.saveDataToFile("DIRECTORCRIPTIONMAKING", sender.getParticipantID(), sender.getUsername() , new Date().getTime(), new Date().getTime(),newLineRemoved, null);       
             ...
             String newLineRemoved = mWYSIWYGkp.getAllTextInWindow().replaceAll("\\r\\n|\\r|\\n", "(NEWLINE)");
             c.saveDataToFile("MATCHERDESCRIPTIONRECEIVED", sender.getParticipantID(), sender.getUsername() , new Date().getTime(), new Date().getTime(),newLineRemoved, null);            
        }
        else{
             String newLineRemoved = (mWYSIWYGkp.getAllTextInWindow().replaceAll("\n", "((NEWLINE))")).replaceAll("\\n", "(NEWLINE)").replaceAll("\\\n", "((NEWLINE))");
     ...
     int firsttestlength =  7;
     
     int secondteststartindex = 8;
     int secondtestlength = 7;
     
     
     
     int  startofnormaltrials = 29;
     ...
     int maxPointsbeforeFinalTest = 800;
