 ...
 * @author sre
 */
public class CCSEQUENCEBACKUP extends DefaultConversationController{

    String ipDDR = "http://"+this.getIPAddress();
    String ipADDRESS = "http://"+this.getIPAddress()+"/01.txt";
    String nullADDRESS = "http://"+this.getIPAddress()+"/blankspace.txt";
...
public void initObjects(){
    ...
    public void processChatText(Participant sender, MessageChatTextFromClient mct) {
            ...
            if(mct.getText().startsWith("/sel")){
                ...
                mct.setText(""+eval);
                if(eval.equalsIgnoreCase("GREEEN"))canMoveToNextStep = true;
            }
