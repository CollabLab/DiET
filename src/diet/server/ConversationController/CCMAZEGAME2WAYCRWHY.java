...
public class CCMAZEGAME2WAYCRWHY  extends DefaultConversationController{
...
public void processChatText(Participant sender,MessageChatTextFromClient mct){    
  ...
  if( !this.carryingOutFakeError&&!processingCRSubdialogue&&
    ...
    ||(lastOriginOfCRDowngrade!=sender && turnsSinceLastIntervention.getValue()>lowerBoundForInter.getValue()))){
                ...
                if(s!=null&&s.length()>0)nounFoundToFrag = true;
                ...
                if(nounFoundToFrag){
                     ...
                     try{Thread.sleep(2500+ 1000*(crText.length()/avgTypingSpeed));}catch (Exception e){}
            ...
            c.informParticipantBthatParticipantAIsTyping(apparentOrigin,sender);
            if(avgTypingSpeed<=0)avgTypingSpeed = 3;
            try{c.sleep(1000+ 1000*(ack.length()/avgTypingSpeed));}catch (Exception e){e.printStackTrace();}
