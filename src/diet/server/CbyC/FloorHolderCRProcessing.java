...
public class FloorHolderCRProcessing extends FloorHolderAdvanced {
	...
	public synchronized void openFloorAfterTimeOut(long timeout) {
		if (fragAppOrig != null) {
			openFloorAfterTimeOutForParticipants(timeout, this.cC.getC()
					.getParticipants().getAllOtherParticipants(fragAppOrig));
		} else
			super.openFloorAfterTimeOut(timeout);
	}

	private synchronized void openFloorAfterTimeOutForParticipants(
			long timeout, Vector participants) {
		...
		if (floorHolder != null) {
			...
			for (int i = 0; i < participants.size(); i++) {
				...
				if (Debug.showCounterAfterCBYCStatusMessage) {
					cC
							.getC()
							.sendLabelDisplayAndEnableToParticipantInOwnStatusWindow(
									p, "Please type " + cCounter, false, true);

				} else {
					cC
							.getC()
							.sendLabelDisplayAndEnableToParticipantInOwnStatusWindow(
									p, "Please type", false, true);
				}
			}
			cCounter++;
			floorHolder = null;
	...
	public void openFloorAfterFragSending() {
		...
		if (fragAppOrig != null) {
			changeFloorStatusOfParticipantsNoPrefix(cC.getC().getParticipants()
					.getAllOtherParticipants(fragAppOrig),
					WYSIWYGDocumentWithEnforcedTurntaking.nooneelsetyping);
			for (Object o : this.cC.getC().getParticipants()
					.getAllOtherParticipants(fragAppOrig)) {
	...
	protected synchronized void giveFloorToParticipantPrefix(Participant sender) {
		...
		if (fragAppOrig != null) {
			...
			if (v.remove(fragAppOrig))
				Conversation.printWSln("FloorHolder", "Successful");
			else
				Conversation.printWSln("FloorHolder", "Unsuccessful");
		}
		...
		if (diet.debug.Debug.showPrefixBeforeCBYCTurn) {
			prefix = sm.getPrefixForParticipant("" + oCounter + "fh", sender,
					sender);
		} else {
		...
		MessageCBYCChangeTurntakingStatus mcbyc = new MessageCBYCChangeTurntakingStatus(
				"server", "server",
				WYSIWYGDocumentWithEnforcedTurntaking.typingunhindered, prefix);
		sender.sendMessage(mcbyc);
		if (Debug.showCounterAfterCBYCStatusMessage) {
			cC.getC().sendAndEnableLabelDisplayToALLStatusWindowsOfParticipant(
					sender, "You are typing " + oCounter, true, true);
		} else {
			cC.getC().sendAndEnableLabelDisplayToALLStatusWindowsOfParticipant(
					sender, "You are typing", true, true);
		}
	...
	protected synchronized void giveFloorToParticipantNoPrefix(
			Participant sender) {
		Conversation.printWSln("Floor Holder", "giveFloor In CR Processing");
		Vector v2 = cC.getC().getParticipants().getAllOtherParticipants(sender);
		Vector v = (Vector) v2.clone();
	...
	public void forceInformOthersOfTyping(Participant actualTypist) {
		infCounter++;
		Vector v2 = cC.getC().getParticipants().getAllOtherParticipants(
				actualTypist);
		Vector v = (Vector) v2.clone();
		if (this.fragAppOrig != null) {
			Conversation.printWSln("FloorHolder",
					"Removing appOrig in inform others");
			v.remove(fragAppOrig);
		}
		if (v.size() == 0)
			Conversation.printWSln("FloorHolder", "sending inform to nobody");
		for (int i = 0; i < v.size(); i++) {
			Participant p = (Participant) v.elementAt(i);
			Conversation.printWSln("FloorHolder", "sending inform to:"
					+ p.getUsername());
			String statusWindowDisplay = sS.getStatusWindowTextForRecipient(p
					.getUsername());
			...
			if (Debug.showCounterAfterCBYCStatusMessage) {
				cC
						...
						.sendAndEnableLabelDisplayToALLStatusWindowsOfParticipant(
								p, statusWindowDisplay + infCounter, true, true);
			} else {
				cC
						.getC()
						.sendAndEnableLabelDisplayToALLStatusWindowsOfParticipant(
								p, statusWindowDisplay, true, true);
			}
		}
	}
}
