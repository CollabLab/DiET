 ...
 * @author Greg
 */
public class MazeGameBufferedSynonymReplacement extends MazeGameDefaultSequence{

    Random r = new Random();
    DynamicSynonymModificationsForDyads dsmfd; 
    CCCBYCMazeGameFakeSynonymReplace cCTypo;
    public boolean performedTransformation = false;
    String target = "NULL";
    String resultOfTransform = "NULL";

    public MazeGameBufferedSynonymReplacement(Sequences sS, CCCBYCDefaultController cC, String sender, Date startTime, String elementText, int eStartPos, int eFinishPos,DynamicSynonymModificationsForDyads dsmfd) {
        super(sS, cC, sender, startTime, elementText, eStartPos, eFinishPos);
        cCTypo = (CCCBYCMazeGameFakeSynonymReplace)cC;
        this.dsmfd=dsmfd;  
    }
    ...
    public MazeGameBufferedSynonymReplacement(Sequences sS, CCCBYCDefaultController cC, String sender, MessageCBYCTypingUnhinderedRequest mCTUR,   DynamicSynonymModificationsForDyads dsmfd) {
    ...
    public void postFIFOEnqueing() {
        ...
        if(index==0){
            ...
            Conversation.printWSln("Main", "Looking at (1) for: "+synonym);
        }
        else if(index==1){
            synonym=this.dsmfd.lookupSynonymsAndRemoveParticipant2(enqWord);
            Conversation.printWSln("Main", "Looking at (2) for: "+synonym);
        }
        else{
        ...
        for(int i=0;i<synonym.length();i++){
            ...
            di.str=""+((synonym.charAt(i)));
            Vector vCmplNEW = new Vector();
            vCmplNEW.addElement(di);
            ...
            fb.insertNoCallBack(vCmplNEW, lastIndexForAddingRemoves);
            lastIndexForAddingRemoves++;
        }
        ...
        for(int i=0;i<synonym.length();i++){
            Conversation.printWSln("Main", "Performing REPLACE "+i);   
        ...
        this.target=enqWord;
        this.resultOfTransform=synonym;
