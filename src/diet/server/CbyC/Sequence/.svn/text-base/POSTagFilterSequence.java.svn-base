...
public class POSTagFilterSequence extends Sequence {
	...
	MaxentTagger tagger;
	String condition;//={"constBoundary" Or "withinConst"}
	Dictionary dict;
	...
	public String getType() {
		if (!this.condition.equals(""))
			return "POSTagFilterSequence(" + condition + ")";
		else
			return "POSTagFilterSequence";
	}

	public POSTagFilterSequence(Sequences sS, CCCBYCDefaultController cC,
			String sender, MessageCBYCTypingUnhinderedRequest mCTUR,
			MaxentTagger tagger, Vector<POSTagRegex> r, Dictionary d,
			Map<String, String> misspellings) {
	...
	public POSTagFilterSequence(Sequences sS, CCCBYCDefaultController cC,
			String sender, Date timeStamp, String elementString,
			int elementStart, int elementFinish, MaxentTagger tagger,
			Vector<POSTagRegex> regexes, Dictionary d,
			Map<String, String> misspellings) {
	...
	public POSTagFilterSequence(Sequences sS, CCCBYCDefaultController cC,
			String sender, Date timeStamp, String elementString,
			int elementStart, int elementFinish, MaxentTagger tagger,
			Vector<POSTagRegex> regexes, Dictionary dict,
			Map<String, String> misspellings, String condition) {
		super(sS, cC, sender, timeStamp, elementString, elementStart,
				elementFinish);
		this.dict = dict;
	...
	protected String fixSpelling() {
		...
		String split[] = this.turnSoFar.split(" ");
		String result = "";
		for (int i = 0; i < split.length; i++) {
	...
	*/
	public String getFragForIntervention()
	{
		...
		ArrayList<? extends HasWord> lastSentence = sentences
				.get(sentences.size() - 1);
		ArrayList<TaggedWord> lastTaggedSentence = tagger
				.tagSentence(lastSentence);
	...
	protected String runFiltersAndGetFrag(ArrayList<TaggedWord> tagged) {
		...
		for (POSTagRegex p : this.regexes) {
			...
			if (m.matches()) {
				...
				Conversation.printWSln("POSTagFilterSeq",
						"Matched. Frag POSSEQ:" + fragPOSGroup);
				String group1 = m.group(1);
				List<String> groupSplit = Arrays.asList(fragPOSGroup.trim()
						.split(" "));
				String frag = findFrag(fragPOSGroup, group1, tagged);
				Conversation.printWSln("POSTagFilterSeq",
						"The Frag (findFrag returns):" + frag);
				List<String> fragSplit = Arrays.asList(frag.toLowerCase()
						.trim().split(" "));
				...
				if (fragSplit.size() == 1
						|| groupSplit.get(groupSplit.size() - 1).equals("NNP"))
					return frag.toLowerCase();

				else if (groupSplit.get(groupSplit.size() - 1).equals("NNS"))
					return frag.toLowerCase();
				/*
				 * String theNoun = fragSplit[fragSplit.length - 1]; String
				 * singular1 = theNoun.substring(0, theNoun .length() - 1);
				 * String singular2 = theNoun.substring(0, theNoun .length() -
				 * 2); if (this.dict.hasWord(singular1) ||
				 * this.dict.hasWord(singular2)) return frag; else return null;
				 * 
				 * }
				 */
				else if (groupSplit.get(groupSplit.size() - 1).equals("DT")) {
					if (frag.equalsIgnoreCase("this")
							|| frag.equalsIgnoreCase("that"))
						return frag.toLowerCase();
					else {
						Conversation
								.printWSln("POSTagFilterSeq",
										"Aborting intervention. Frag is determiner but not this/that");
						return null;
					}
				}
					...
					if (this.dict.hasWord(fragSplit.get(fragSplit.size() - 1)))
						return frag;
					else

					{
						Conversation
								.printWSln("POSTagFilterSeq",
										"Aborting intervention. Frag end not in dicitonary.");
						return null;
					}
				}
	...
	protected String getTagSequence(ArrayList<TaggedWord> lastSent, String endInTag) {
		...
		for (int i = 0; i < lastSent.size(); i++) {
			...
			if (i == lastSent.size() - 1
					&& !endInTag.equalsIgnoreCase("EndsInTag"))
				result += word.word();
			else
				result += (word.tag() + " ");
		}
