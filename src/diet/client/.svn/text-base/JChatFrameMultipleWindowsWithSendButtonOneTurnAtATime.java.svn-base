 ...
 * @author user
 */
public class JChatFrameMultipleWindowsWithSendButtonOneTurnAtATime extends JChatFrame {
    ...
    int numberOfTurnsDisplayed;
    boolean isScrollable;

    public JChatFrameMultipleWindowsWithSendButtonOneTurnAtATime(ClientEventHandler clevh, int numberOfWindows, int numberOfRows, int numberOfColumns,boolean isVertical,boolean hasStatusWindow,int windowOfOwnText, int numberOfRowsInTextEntry,int numberOfTurnsDisplayed, boolean isScrollable) {
        super (clevh);
        try {
        ...
        this.pack();
        this.validate();
        }catch (Exception ex){System.err.println("COULD NOT SET UP CHAT INTERFACE");}
    ...
    class JCollectionOfChatWindows extends JPanel{
        ...
        public JCollectionOfChatWindows(int numberOfWindows,int numberOfRows,int numberOfColumns,boolean isVertical,boolean hasStatusWindow,int windowOfParticipant){
            ...
            for(int i=0;i<numberOfWindows;i++){
                JTextAreaOneTurnAtATime jTexta = new JTextAreaOneTurnAtATime(numberOfRows,numberOfColumns);
